#!/bin/bash
# Run local_loop.py (gen 6, local Qwen) with the memory it needs: waits for any Song Forge song to
...
#   ./run_local_loop.sh "DAWN RIDGES"            one subject
#   ./run_local_loop.sh --all                     all eleven, holdouts painted once
#   ./run_local_loop.sh --resume local_runs/<dir> carry on after an interruption
set -u
cd "$(dirname "$0")"
...
LEASE="$(/usr/bin/python3 "$MEM" wait qwen-paint 48 --timeout 300 2>/dev/null)"
[ -n "$LEASE" ] && echo "memory seat $LEASE" || echo "forge_guard gave no seat — running anyway"

HF_HUB_OFFLINE=1 PYTHONWARNINGS=ignore "$HOME/.local/mlx-vlm-latest/bin/python3" local_loop.py "$@" &
PID=$!
