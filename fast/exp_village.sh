...
# seat as run_director.sh, taken ONCE for all six runs. Log: experiments/village/run.log
set -u
cd "$(dirname "$0")"
...
for spec in \
  ...
  for mode in blind seeing; do
    ...
    ( trap - EXIT INT TERM      # the background copy must never run restore()
      MATTPAINT_NAME="$NAME" MATTPAINT_TAG="${TAG}_$mode" MATTPAINT_MODEL="$MODEL" HF_HUB_OFFLINE=1 PYTHONWARNINGS=ignore \
