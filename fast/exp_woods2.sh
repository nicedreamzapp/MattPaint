...
for spec in \
  ...
  for mode in blind; do
    ...
    ( trap - EXIT INT TERM      # the background copy must never run restore()
      MATTPAINT_NAME="$NAME" MATTPAINT_TAG="${TAG}_woods" MATTPAINT_MODEL="$MODEL" HF_HUB_OFFLINE=1 PYTHONWARNINGS=ignore \
        exec "$HOME/.local/mlx-vlm-latest/bin/python3" director.py "$PROMPT" --rounds 1 --no-think ${extra[@]+"${extra[@]}"} ) &
    PID=$!
